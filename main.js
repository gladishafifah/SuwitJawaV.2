...
function infoBG(hasil) {
  if (hasil === 'SERI') {
    ...
    info.style.backgroundColor = '#55cd4c';
  } else if (hasil === 'KALAH..') {
    info.classList.remove('text-black');
...
function noteComp() {
  const text = Math.random();
    ...
    else if(text >= 0.2 && text < 0.4) return 'Kamu pasti bisa!';
    else if(text >= 0.4 && text < 0.6) return 'Jangan Menyerah!';
      return 'Jangan mau kalah sama computer';
}

function notePlayer() {
  const text = Math.random();
    if(text < 0.2) return 'Keren!';
    else if(text >= 0.2 && text < 0.4) return 'Computernya kena mental';
    else if(text >= 0.4 && text < 0.6) return 'You are extraodinary';
